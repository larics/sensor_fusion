...
public:
  Imu(const ModelCovariance& cov, ros::NodeHandle& nh_private)
    : init_(false), fresh_measurement_(false), delta_t_(0.0), Q_f_(cov.Q_f), Q_w_(cov.Q_w)
  {
  ...
  void callback(const sensor_msgs::ImuPtr& msg)
  {
    if (!init_) {
      ...
      imu_w_ << msg->angular_velocity.x, msg->angular_velocity.y, msg->angular_velocity.z;
