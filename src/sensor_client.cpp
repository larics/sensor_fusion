...
SensorClient::SensorClient(const EsEkfParams& params,
				...
				params_(params),es_ekf_(),start_flag_(true),
				start_camera_imu_(true),start_imu_(true),
	...
	camera_acc_sub_ = node_handle_.subscribe(acc_topic, 1,
																					 &SensorClient::camera_acc_callback, this);
	camera_odom_sub_ = node_handle_.subscribe(camera_odom_topic, 1,
																						&SensorClient::camera_odom_callback, this);
	camera_gyro_sub_ = node_handle_.subscribe(gyro_topic, 1,
																						&SensorClient::camera_gyro_callback, this);
	...
	update_timer_ = node_handle_.createTimer
					(ros::Duration(0.01),
					 &SensorClient::state_estimation,this);
...
//						<< "\nNorm-> " << (measurement-es_ekf_.getPose()).norm() << '\n';
	return ((measurement-es_ekf_.getPose()).norm() < outlier_constant_);
}
...
void SensorClient::state_estimation(const ros::TimerEvent &msg) {
	...
	if (start_flag_){
		if (camera_odom_ready()){
			...
			es_ekf_.setPose(this->get_camera_pose());
			es_ekf_.setQuat(this->get_camera_orientation());
			es_ekf_.setVel(this->get_camera_lin_vel());
			start_flag_ = false;
		}
		return;
	}
	...
	if(params_.camera.use_camera_imu &&
		...
		es_ekf_.prediction( cam_imu_to_cam * this->get_acc(), params_.model.Q_f,
											 cam_imu_to_cam * this->get_angular_vel(), params_.model.Q_w,
												delta_t_);
		prediction = true;
	}
	...
	if (this->camera_odom_ready()){
		if(outlier_detection(es_ekf_.getCameraOrientation().toRotationMatrix() *
													this->get_camera_pose() + es_ekf_.getCameraTranslation())) {
			measurement = true;
