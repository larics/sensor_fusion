...
EsEkfParams parse_yaml(const std::string& config_file)
{
  ...
  for (size_t i = 0; i < id.size(); i++) {
    ...
    int is_orient_sensor         = config[id.at(i) + "_is_orientation_sensor"].as<int>();
    ...
    sensor.translation << translation.at(0), translation.at(1), translation.at(2);
    std::cout << "id.at(i)-> " << id.at(i) << '\n';
    sensor.estimate_drift = config[id.at(i) + "_estimate_drift"].as<int>();
    sensor.origin_at_first_measurement =
      config[id.at(i) + "_origin_at_first_measurement"].as<int>();
