...
struct EsEkfParams {
  ...
  Translation3d g;
  bool estimate_acc_bias, estimate_gyro_bias, estimate_gravity_bias;
...
class EsEkfState {
 ...
 public:
  EsEkfState() { state = MatrixXd::Zero(10, 1); }
  Matrix<double, 10, 1> getState() const { return state; }
  void operator=(const nav_msgs::OdometryPtr& msg) {
    this->state << msg->pose.pose.position.x, msg->pose.pose.position.y,
