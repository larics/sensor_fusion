...
public:
		Camera(EsEkfParams params, EsEkf* es_ekf,
			...
			pose_sub = node_handle_.subscribe("/pozyx/measured", 1,
		...
		void acc_callback(const sensor_msgs::Imu& msg){
			...
			ekf_pose_.twist.twist.angular.x   = pose_posix_[0];
			...
			if (gyro_init_){
				...
				acc_ = rotation_matrix_ * acc_; // TO camera frame
				angular_vel_ = rotation_matrix_ * angular_vel_;// To camera frame
				//acc_ = params_.camera.rotation_mat * acc_;
			...
			if(odom_init_){
				odom_init_ = false;
				//pose_ = params_.camera.rotation_mat * pose_
				...
				if (params_.camera.is_odom){
					...
					std::cout << "odom_pose_ -> " << pose_.transpose() << '\n'
										<< "est pose   -> " << (es_ekf_->getPose() ).transpose()<< '\n'
		...
		void odom_callback(const nav_msgs::Odometry& msg){
			...
			if (first_camera_measurement_){
				ROS_INFO("FIRST MEASUREMNT");
...
//				if (std::abs(pose_[0]) < 1e-3) pose_[0] = 0;
//				if (std::abs(pose_[1]) < 1e-3) pose_[1] = 0;
//				if (std::abs(pose_[2]) < 1e-3) pose_[2] = 0;
				old_pose_ << msg.pose.pose.position.x,
										 msg.pose.pose.position.y,
										 msg.pose.pose.position.z;
			}
...
//			linear_vel_ = params_.camera.rotation_mat * linear_vel_;
		...
		void 	pose_callback(const geometry_msgs::TransformStamped& msg){
			...
			pose_posix_ = params_.camera.rotation_mat.transpose() * pose_posix_
										- params_.camera.translation;
